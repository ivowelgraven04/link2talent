...
import { ArrowRight, UserCircle, Lightning, EnvelopeSimple } from "@phosphor-icons/react/dist/ssr";
...
const services: Service[] = [
  ...
  },
  {
    ...
    description: "Betaling op basis van resultaat. Alleen voor bewezen proposities.",
    items: ["Betaling op basis van resultaat", "Alleen voor bewezen proposities"],
    extra: { label: "Uitbreiding mogelijk", items: ["Extra setters en Closers"] },
    result: "Schaal op wanneer het werkt.",
            ...
            const inner = (
              <div className="relative flex flex-col gap-6 rounded-2xl p-8 border h-full bg-white/[0.03] border-white/[0.08] hover:border-[#007BFF]/50 hover:bg-white/[0.05] transition-all duration-200 cursor-pointer group">
                <div>
                  ...
                  {s.extra && (
                    <div className="mt-5">
                      <p className="text-white/40 text-xs font-semibold uppercase tracking-[0.15em] mb-2">{s.extra.label}</p>
                      <div className="flex flex-col gap-2">
                        {s.extra.items.map((item) => (
                          <div key={item} className="flex items-start gap-2.5">
                            <div className="w-1 h-1 rounded-full bg-[#007BFF] mt-2 shrink-0" />
                            <span className="text-white/65 text-sm">{item}</span>
                          </div>
